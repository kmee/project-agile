...
class TestUserStory(TransactionCase):
    ...
    def test_salesman_configuration_contrac(self):
        ...
        # Validate if salesman does not finish the configuration the customer
        # will not watch the true portal
        project_1 = self.data.get_object(
            cr, uid, 'project', 'project_project_5')
        self.assertRaises(except_orm, self.project.read, cr, daniel_user.id,
                          project_1.id, [])
        # Salesman configures correctly the portal access setting as follower
        # to the project user
    ...
    def test_customer_changes_user_story(self):
        ...
        charlie_user = self.data.get_object(
            cr, uid, 'portal_user_story', 'user_story_demo_user_2')
